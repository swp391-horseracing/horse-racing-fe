...
export default function OwnerPage() {
  ...
  const handleAddHorse = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (horses.some((h) => h.name.toLowerCase() === name.toLowerCase() && h.status !== "Retired")) {
      addToast("Duplicate Name: An active horse already shares this name.", "error");
      return;
    }

    try {
      await addHorse({ name, breed, dob, gender, microchipId, associationCode });
  ...
  const handleRetireHorse = async (id: number) => {
    if (isHorseLocked(id)) {
      addToast("Action Blocked: Horse has active tournament commitments.", "warning");
      return;
    }
  ...
  const handleRegisterTournament = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    ...
    if (horse.breed !== tournament.allowedBreed) {
      addToast(`Eligibility Failure: Requires breed "${tournament.allowedBreed}".`, "error");
      return;
    }

    const age = calculateAge(horse.dob);
    if (age < tournament.minAge || age > tournament.maxAge) {
      addToast(`Eligibility Failure: Age must be ${tournament.minAge}-${tournament.maxAge} years.`, "error");
      return;
    }

    const isFull = tournament.currentCount >= tournament.maxCapacity;
    const status: "Pending Approval" | "Waitlisted" = isFull ? "Waitlisted" : "Pending Approval";

    try {
      await registerTournament(selectedHorseId, selectedTournamentId, status);
      setShowRegisterTournament(false);
      addToast(
        isFull ? "Tournament full. Joined waitlist successfully." : "Registration submitted for Admin approval.",
  ...
  const handleInviteJockeys = async (jockeyIds: number[], tournamentId: number, horseId: number) => {
    if (jockeyIds.length === 0) return;

    try {
      await inviteJockeys(jockeyIds, tournamentId, horseId);
      setShowInviteJockey(false);
      addToast(`Dispatched invitations to ${jockeyIds.length} Jockey(s).`, "success");
    } catch {
    ...
    if (loading) {
      return (
        <div className="flex items-center justify-center h-64 text-sm text-slate-400 font-body">
          <Clock className="w-5 h-5 animate-spin mr-2" /> Loading stable records...
        </div>
      );
    }
          ...
          {toasts.map((t) => (
            <div
              ...
              className={cn(
                ...
                t.type === "info" && "bg-indigo-50 border-indigo-200 text-indigo-955"
              )}
            >
              ...
              {t.type === "info" && <Activity className="w-4 h-4 text-indigo-600 shrink-0" />}
              <span>{t.message}</span>
            </div>
          ))}
        </div>
          ...
          <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4">
            <div className="bg-white rounded-xl border border-slate-100 p-5 max-w-md w-full shadow-xl font-body">
              <div className="flex items-center justify-between border-b pb-2.5 mb-4">
                <h3 className="font-bold text-base text-[#064E3B]">Add Horse to Roster</h3>
                <button onClick={() => setShowAddHorse(false)} className="text-slate-400 hover:text-slate-655 text-sm">
                  ✕
                </button>
              </div>
              <form onSubmit={handleAddHorse} className="space-y-3.5">
                <div className="grid grid-cols-2 gap-3.5">
                  <div className="col-span-2">
                    <label className="text-[10px] uppercase font-bold text-slate-500 block mb-1">Horse Name</label>
                    <input
                      required
                      name="name"
                      type="text"
                  ...
                  <div>
                    <label className="text-[10px] uppercase font-bold text-slate-500 block mb-1">Breed</label>
                    <select
                      required
                      name="breed"
                      className="w-full bg-slate-50 border rounded-md p-2 text-xs outline-hidden focus:border-[#064E3B]"
                  ...
                  <div>
                    <label className="text-[10px] uppercase font-bold text-slate-500 block mb-1">Date of Birth</label>
                    <input
                      required
                      name="dob"
                      type="date"
                  ...
                  <div className="col-span-2">
                    <label className="text-[10px] uppercase font-bold text-slate-500 block mb-1">Breeding Association Code</label>
                    <input
                      required
        ...
        {showRegisterTournament && (
          <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4">
            <div className="bg-white rounded-xl border border-slate-100 p-5 max-w-sm w-full shadow-xl font-body">
              <div className="flex items-center justify-between border-b pb-2.5 mb-4">
                <h3 className="font-bold text-base text-[#064E3B]">Tournament Registration</h3>
                <button onClick={() => setShowRegisterTournament(false)} className="text-slate-400 hover:text-slate-650 text-sm">
                  ✕
                </button>
              </div>
              <form onSubmit={handleRegisterTournament} className="space-y-4">
                <div>
                  <label className="text-[10px] uppercase font-bold text-slate-500 block mb-1">Select Stable Entry</label>
                  <select
                  ...
                  >
                    <option value="" disabled>-- Choose Horse --</option>
                    {horses
                ...
                <div>
                  <label className="text-[10px] uppercase font-bold text-slate-500 block mb-1">Target Tournament</label>
                  <select
          ...
          <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4">
            <div className="bg-white rounded-xl border border-slate-100 p-5 max-w-md w-full shadow-xl font-body">
              <div className="flex items-center justify-between border-b pb-2.5 mb-3">
                <h3 className="font-bold text-base text-[#064E3B]">Select Jockeys to Invite</h3>
                <button onClick={() => setShowInviteJockey(false)} className="text-slate-400 hover:text-slate-655 text-sm">
                  ✕
                </button>
              </div>
              <JockeyInviteSelector
                ...
                onDispatch={(selectedIds) => {
                  handleInviteJockeys(selectedIds, selectedTournamentId, selectedHorseId);
                }}
              />
            </div>
          </div>
...
function DashboardOverview({
  ...
  return (
    <div className="p-5 space-y-5 max-w-6xl mx-auto font-body">
      <div>
        <h2 className="text-xl font-black text-[#064E3B] tracking-tight">Owner Dashboard</h2>
        <p className="text-xs text-slate-550 mt-1">Manage stable profiles, registrations, and jockey invitations.</p>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: "Active Stable", val: `${activeHorsesCount} Horses`, action: () => setActiveTab("/owner/horseManagement") },
          { label: "Open Events", val: `${tournaments.filter((t) => t.status === "Registration Open").length} Active`, action: () => setActiveTab("/owner/raceRegister") },
          { label: "Pending Approvals", val: `${pendingRegCount} Queued`, action: () => setActiveTab("/owner/raceRegister") },
          { label: "Pending Invites", val: `${pendingInvCount} Sent`, action: () => setActiveTab("/owner/jockeys") },
        ].map((card, idx) => (
          <div
            key={idx}
            onClick={card.action}
          ...
          >
            <span className="text-[10px] text-slate-400 uppercase font-bold block">{card.label}</span>
            <span className="text-base font-extrabold text-[#064E3B] block mt-1.5">{card.val}</span>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div className="bg-white border rounded-xl p-4 lg:col-span-2 shadow-xs space-y-3">
          <div className="border-b pb-2 flex justify-between items-center">
            <h3 className="font-bold text-sm text-[#064E3B]">Stable Roster Overview</h3>
            <button onClick={() => setActiveTab("/owner/horseManagement")} className="text-[10px] font-bold text-indigo-600 hover:underline">
              Manage Stables
            </button>
          </div>
          <div className="divide-y text-xs">
            {horses
              ...
              .map((horse) => (
                <div key={horse.id} className="py-2.5 flex items-center justify-between">
                  <div>
                    <p className="font-bold text-slate-850">{horse.name}</p>
                    <p className="text-slate-400 text-[10px]">{horse.breed} • {horse.gender}</p>
                  </div>
        ...
        <div className="bg-white border rounded-xl p-4 shadow-xs space-y-3">
          ...
          <div className="space-y-2">
            {jockeys.slice(0, 3).map((j) => (
              <div key={j.id} className="p-2.5 bg-slate-50/50 rounded-lg flex items-center justify-between text-xs border border-slate-100">
                <div>
                  <p className="font-bold text-slate-850">{j.name}</p>
                  <p className="text-[10px] text-slate-400">{j.club}</p>
                </div>
                <div className="text-right">
                  <span className="font-extrabold text-[#064E3B] block">{j.winRate} WR</span>
                  <span className="text-[9px] text-slate-400 block">{j.totalRuns} Starts</span>
                </div>
              </div>
            ))}
          </div>
  ...
  return (
    <div className="p-5 space-y-5 max-w-6xl mx-auto font-body">
      <div className="flex items-center justify-between border-b pb-4">
        <div>
          <h2 className="text-xl font-black text-[#064E3B] tracking-tight">Horse Registry</h2>
          <p className="text-xs text-slate-555 mt-1">Manage your stable profiles, view horse details, and retire active horses.</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {horses
          ...
          .map((horse) => {
            ...
            return (
              <div key={horse.id} className="bg-white border rounded-xl p-4.5 shadow-xs flex flex-col justify-between hover:shadow-md transition">
                <div className="space-y-3">
                  <div className="flex items-start justify-between gap-2">
                    <h3 className="font-bold text-sm text-[#064E3B] truncate">{horse.name}</h3>
                    <span
                      className={cn(
                        "text-[8px] font-black uppercase px-2 py-0.5 rounded-full border shrink-0",
                        locked ? "bg-amber-50 border-amber-200 text-amber-800" : "bg-emerald-50 border-emerald-200 text-emerald-800"
                      )}
                    >
                  ...
                  <div className="grid grid-cols-2 gap-2 text-[10px] pb-1">
                    <div>
                      <span className="text-slate-400 block font-semibold uppercase text-[8px]">Breed</span>
                      <span className="font-bold text-slate-700">{horse.breed}</span>
                    </div>
                    <div>
                      <span className="text-slate-400 block font-semibold uppercase text-[8px]">Gender</span>
                      <span className="font-bold text-slate-700">{horse.gender}</span>
                    </div>
                  </div>
                </div>
...
type TournamentFilter = "All" | "Live now" | "Registration open" | "Upcoming" | "Completed";

const STATUS_FILTER_MAP: Record<TournamentFilter, Tournament["status"][]> = {
  All: ["Registration Open", "Registration Closed", "Scheduled", "Live", "Concluded"],
  "Live now": ["Live"],
...
function RaceRegister({
  ...
  const FILTER_TABS: TournamentFilter[] = ["All", "Live now", "Registration open", "Upcoming", "Completed"];
  ...
  const getStatusBadge = (status: Tournament["status"]) => {
    switch (status) {
      case "Live":
        return <span className="text-[11px] font-bold px-3 py-1 rounded-full bg-blue-600 text-white shadow-sm">Live now</span>;
      case "Registration Open":
        return <span className="text-[11px] font-bold px-3 py-1 rounded-full bg-emerald-600 text-white shadow-sm">Registration open</span>;
      case "Scheduled":
      case "Registration Closed":
        return <span className="text-[11px] font-bold px-3 py-1 rounded-full bg-slate-100 text-slate-600 border border-slate-200">Upcoming</span>;
      case "Concluded":
        return <span className="text-[11px] font-bold px-3 py-1 rounded-full bg-slate-100 text-slate-500 border border-slate-200">Completed</span>;
      default:
        return null;
    }
  };
  ...
  const getTopRightLabel = (t: Tournament) => {
    if (t.status === "Live") return <span className="text-[12px] text-slate-505 font-medium">Round 1 of 3</span>;
    ...
    if (t.status === "Scheduled" || t.status === "Registration Closed") {
      return <span className="text-[12px] text-slate-505 font-medium">Reg. opens in 14d</span>;
    }
    return <span className="text-[12px] text-slate-555 font-medium">Dec 2025</span>;
  };
        ...
        <div className="bg-[#064E3B]/5 border border-[#064E3B]/10 rounded-xl p-3.5 flex items-center justify-between">
          {horse ? (
            <span className="text-[12px] text-[#064E3B] font-bold flex items-center gap-1.5">
              <Zap className="w-4 h-4 text-[#064E3B]" /> {horse.name} · Race 3 at 14:30 today
            </span>
          ) : (
            <span className="text-[12px] text-slate-505 font-medium">No active stable entries</span>
          )}
      ...
      return (
        <div className="bg-slate-50 border border-slate-100 rounded-xl p-3.5 flex items-center justify-between">
          <span className="text-[12px] text-slate-600 font-medium">
            {horse ? `${horse.name} — 1st place · $12,000 earned` : "Tournament concluded"}
          </span>
  ...
  return (
    <div className="p-5 space-y-5 max-w-6xl mx-auto font-body">
      <div className="flex items-center justify-between border-b pb-4">
        <div>
          <h2 className="text-xl font-black text-[#064E3B] tracking-tight">Race & Tournament Registration</h2>
          <p className="text-xs text-slate-555 mt-1">Register your active horses for upcoming events and monitor application statuses.</p>
        </div>
      </div>
      ...
      {filteredTournaments.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredTournaments.map((t) => {
            ...
            return (
              <div key={t.id} className="bg-white rounded-2xl border border-slate-200 p-5 flex flex-col hover:shadow-md transition-all duration-200">
                ...
                <div className="mb-4">
                  <h3 className="font-black text-[17px] tracking-tight text-[#064E3B] mb-2 leading-tight">{t.name}</h3>
                ...
                {isOpen && (
                  <div className="mb-4">
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-[12px] text-slate-505 font-medium">{t.currentCount} / {t.maxCapacity} horses registered</span>
                      <span className="text-[12px] font-bold text-emerald-600">{slotsLeft} slots left</span>
                    </div>
                ...
                <div className="grid grid-cols-2 gap-4 mb-5">
                  {[
                    ...
                    { label: isOpen ? "Rounds" : isConcluded ? "Horses" : "Max horses", value: isOpen ? "3" : isConcluded ? `${t.currentCount}` : `${t.maxCapacity}` },
                  ].map((stat) => (
                    <div key={stat.label}>
                      <span className="block text-[11px] font-medium text-slate-505 mb-1">{stat.label}</span>
                      <span className="block text-[15px] font-black text-slate-800">{stat.value}</span>
                    </div>
                  ))}
                </div>
  ...
  return (
    <div className="p-5 space-y-5 max-w-6xl mx-auto font-body">
      <div className="border-b pb-4">
        <h2 className="text-xl font-black text-[#064E3B] tracking-tight">Jockey Roster</h2>
        <p className="text-xs text-slate-555 mt-1">Hire jockeys for approved tournament runs and finalize pairings.</p>
      </div>
        ...
        {approvedRegistrations.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {approvedRegistrations.map((reg) => {
              ...
              const matchingInvites = invitations.filter((i) => i.horseId === reg.horseId && i.tournamentId === reg.tournamentId);
              const lockedJockey = matchingInvites.find((i) => i.status === "Accepted");

              if (!horse || !tournament) return null;

              return (
                <div key={reg.id} className="bg-white border rounded-xl p-4 shadow-xs space-y-3.5 flex flex-col justify-between">
                  <div className="space-y-3">
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <h4 className="font-bold text-sm text-[#064E3B]">{horse.name}</h4>
                        <p className="text-[10px] text-slate-455 leading-tight mt-0.5">{tournament.name}</p>
                      </div>
                      {lockedJockey ? (
                    ...
                    <div className="space-y-1.5 pt-2 border-t text-xs">
                      <span className="text-[9px] font-extrabold uppercase text-slate-400 tracking-wider block">Sent Invitations</span>
                      {matchingInvites.length === 0 ? (
                        <p className="text-[10px] text-slate-400 italic">No invitations sent yet.</p>
                      ) : (
                        <div className="space-y-1.5">
                          {matchingInvites.map((inv) => {
                            const jockey = jockeys.find((j) => j.id === inv.jockeyId);
                            return (
                              <div key={inv.id} className="p-2 bg-slate-50 border rounded-lg flex items-center justify-between">
                                <div>
                                  <p className="font-bold text-slate-700 text-xs">{jockey?.name}</p>
                                  <span className="text-[9px] text-slate-400">Club: {jockey?.club}</span>
                                </div>
                                <div className="flex items-center gap-2">
                                  <span
                                    className={cn(
                                      "text-[8px] font-black uppercase px-1.5 py-0.5 rounded border",
                                      inv.status === "Accepted" && "bg-emerald-50 border-emerald-200 text-emerald-855",
                                      inv.status === "Pending" && "bg-amber-50 border-amber-200 text-amber-855",
                                      inv.status === "Declined" && "bg-rose-50 border-rose-200 text-rose-855",
                                      inv.status === "Superseded" && "bg-slate-100 border-slate-200 text-slate-455"
                                    )}
                                  >
                                    {inv.status}
                                  </span>
  ...
  return (
    <div className="space-y-3">
          ...
          return (
            <div
              ...
              className={cn(
                ...
                isSelected ? "bg-emerald-50/50 border-[#064E3B]" : "bg-white border-transparent hover:bg-slate-50"
              )}
            >
              <div className="flex items-center gap-2">
                <input
              ...
              <div className="text-right text-[11px]">
                <span className="font-bold text-[#064E3B] block">{j.winRate} WR</span>
                <span className="text-[9px] text-slate-455 block">{j.totalRuns} Matches</span>
              </div>
            </div>
          );
        })}
      ...
      <div className="border-t pt-3 flex items-center justify-between">
        <span className="text-[10px] text-slate-555 font-semibold">{selectedIds.length} Selected</span>
        <button
...
const RegStatusBadge = ({ status }: { status: TournamentRegistration["status"] }) => {
  const map: Record<TournamentRegistration["status"], { cls: string; label: string }> = {
    Approved: { cls: "bg-emerald-50 border-emerald-200 text-emerald-800", label: "Approved" },
    "Pending Approval": { cls: "bg-amber-50 border-amber-200 text-amber-800", label: "Pending" },
    Waitlisted: { cls: "bg-indigo-50 border-indigo-200 text-indigo-800", label: "Waitlisted" },
    Withdrawn: { cls: "bg-rose-50 border-rose-200 text-rose-800", label: "Withdrawn" },
  };
  const cfg = map[status] ?? map["Pending Approval"];
  return (
    <span className={cn("text-[8px] font-black uppercase px-2 py-0.5 rounded border shrink-0", cfg.cls)}>
      {cfg.label}
    </span>
  );
};
    ...
    >
      <div className="flex items-center justify-between w-full gap-3">
        <div className="flex items-center gap-3 min-w-0">
          <span className={cn("font-mono text-sm tracking-tight font-black shrink-0", selected ? "text-[#064E3B]" : "text-slate-400")}>
            {mockRaceTime(entry.tournament.id)}
          </span>
          <div className="truncate">
            <p className={cn("font-bold font-headline text-sm truncate", selected ? "text-[#064E3B]" : "text-slate-800")}>
              {entry.horse.name}
            </p>
            <p className="text-[10px] text-slate-500 mt-0.5 truncate font-medium">{entry.tournament.name}</p>
          </div>
        </div>
...
function ScheduleDetailPanel({
  ...
  return (
    <ScheduleDetailFrame
      ...
      subtitle={
        <p className="text-white/60 text-xs font-semibold mt-0.5">{tournament.name}</p>
      }
        ...
        noJockey ? (
          <div className="flex items-center gap-3 rounded-xl bg-[#D97706]/20 border border-[#D97706]/40 px-4 py-2.5">
            ...
            <p className="text-xs font-bold text-[#EAB308] flex-1">
              No jockey assigned — your horse will be listed as <span className="uppercase">Scratched</span> unless you invite a jockey before the deadline.
            </p>
            <button
              onClick={() => onOpenInviteModal(horse.id, tournament.id)}
    ...
    >
      ...
      {activeTab === "info" && (
        <>
          <div>
            <h3 className="text-[10px] font-black uppercase tracking-widest text-[#064E3B]/60 mb-3">Your Entry</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="p-4 bg-white border border-[#064E3B]/10 rounded-xl shadow-sm">
                ...
                <span className="text-xs text-slate-555 mt-0.5 block">{horse.breed} · {horse.gender}</span>
              </div>
              <div className="p-4 bg-white border border-[#064E3B]/10 rounded-xl shadow-sm">
          ...
          <div>
            <h3 className="text-[10px] font-black uppercase tracking-widest text-[#064E3B]/60 mb-3">Event Details</h3>
            <div className="bg-white border border-[#064E3B]/10 rounded-xl overflow-hidden shadow-sm">
              <div className="divide-y divide-slate-100 text-sm">
                {[
                  { icon: <Flag className="w-4 h-4" />, label: "Allowed Breed", value: tournament.allowedBreed },
                  { icon: <Activity className="w-4 h-4" />, label: "Age Range", value: `${tournament.minAge} – ${tournament.maxAge} years` },
                  { icon: <Users className="w-4 h-4" />, label: "Capacity", value: `${tournament.currentCount} / ${tournament.maxCapacity} horses` },
                  { icon: <Trophy className="w-4 h-4" />, label: "Prize Fund", value: "$175,000" },
                ].map((row) => (
          ...
          {invitations.length > 0 && (
            <div>
              <h3 className="text-[10px] font-black uppercase tracking-widest text-[#064E3B]/60 mb-3">Invitation Status</h3>
              <div className="space-y-2">
                {invitations.map((inv) => (
                  <div key={inv.id} className="flex items-center justify-between px-4 py-2.5 bg-[#F4F6F5] rounded-xl border border-slate-100 text-xs">
                    <span className="font-semibold text-slate-700">Jockey #{inv.jockeyId}</span>
                    <span
                      className={cn(
                        "text-[8px] font-black uppercase px-2 py-0.5 rounded border",
                        inv.status === "Accepted" && "bg-emerald-50 border-emerald-200 text-emerald-800",
                        inv.status === "Pending" && "bg-amber-50 border-amber-200 text-amber-800",
                        inv.status === "Declined" && "bg-rose-50 border-rose-200 text-rose-800",
                        inv.status === "Superseded" && "bg-slate-100 border-slate-200 text-slate-500"
                      )}
                    >
                      {inv.status}
                    </span>
          ...
          <div className="overflow-hidden rounded-xl border border-[#064E3B]/10 bg-white shadow-sm">
            <table className="w-full text-left">
              <thead className="bg-[#F4F6F5] border-b border-slate-100">
                <tr>
                  <th className="px-5 py-3.5 text-[9px] font-black uppercase tracking-widest text-slate-400 w-16 text-center">Cloth</th>
              ...
              <tbody className="divide-y divide-slate-100 text-sm">
                {runners.map((runner, idx) => {
                  ...
                  const isPending = isOurs && runner.jockeyName === "— Invite Pending —";

                  return (
                    <tr key={idx} className={cn("transition-colors", isOurs ? (isScratched ? "bg-rose-50/60" : "bg-[#064E3B]/5") : "hover:bg-slate-50/50")}>
                      ...
                      <td className="px-5 py-3.5">
                        <span className={cn("font-bold font-headline text-base leading-snug", isOurs ? "text-[#064E3B]" : "text-slate-800", isScratched && "line-through text-rose-400")}>
                          {runner.horseName}
                        </span>
                        {isOurs && (
                      ...
                      <td className="px-5 py-3.5 font-medium text-slate-600">
                        {isScratched ? <span className="text-rose-400 italic text-xs">Not assigned</span> : isPending ? <span className="text-indigo-500 font-semibold text-xs italic">— Invite Pending —</span> : runner.jockeyName}
                      </td>
                      <td className="px-5 py-3.5 text-right">
                        {isScratched ? (
                          ...
                          <span className="text-[9px] font-black uppercase px-2 py-0.5 rounded border bg-indigo-50 border-indigo-200 text-indigo-700">Pending</span>
                        ) : (
                          <span className="text-[9px] font-black uppercase px-2 py-0.5 rounded border bg-emerald-50 border-emerald-200 text-emerald-700">Confirmed</span>
                        )}
                      </td>
                    </tr>
                  );
...
export function HorseScheduleView({
  ...
  const [search, setSearch] = useState("");
  const [statusFilter, setStatusFilter] = useState<"All" | "no-jockey" | "pending-invite" | "confirmed">("All");
  ...
  const allEntries = useMemo<ScheduleEntry[]>(() => {
    return registrations
      ...
      .map((reg) => {
        ...
        const regInvitations = invitations.filter((i) => i.horseId === reg.horseId && i.tournamentId === reg.tournamentId);
        const confirmedInv = regInvitations.find((i) => i.status === "Accepted");
        const confirmedJockey = confirmedInv ? (jockeys.find((j) => j.id === confirmedInv.jockeyId) ?? null) : null;
        const hasPendingInvite = regInvitations.some((i) => i.status === "Pending");

        const rawDate = (tournament as Tournament & { startDate?: string }).startDate ?? "2026-06-15";
  ...
  const raceDays = useMemo(() => allEntries.map((e) => parseLocalDate(e.dateKey)), [allEntries]);

  const selectedEntry = useMemo(() => allEntries.find((e) => e.regId === selectedEntryId) ?? null, [allEntries, selectedEntryId]);
    ...
    <div className="flex-1 overflow-y-auto p-6 max-w-7xl w-full mx-auto font-body h-full custom-scrollbar bg-[#F4F6F5]">
      <div className="flex-shrink-0">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <div>
            <h2 className="text-xl font-bold font-headline text-[#064E3B]">Run Schedule</h2>
        ...
        <div className="mb-6 grid grid-cols-2 md:grid-cols-4 gap-3">
          {(["All", "no-jockey", "pending-invite", "confirmed"] as const).map((key) => {
            const labels = { All: "All Races", "no-jockey": "No Jockey", "pending-invite": "Invite Sent", confirmed: "Confirmed" };
            return (
              <ScheduleStatCard
                key={key}
                label={labels[key]}
        ...
        listSlot={
          <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden flex flex-col">
            <div className="border-b border-slate-100 bg-[#F4F6F5] px-5 py-3 flex items-center gap-2">
              ...
              {selectedDate && (
                <button onClick={() => setSelectedDate(undefined)} className="ml-auto text-[9px] text-slate-400 hover:text-slate-655 font-bold">
                  Clear
                </button>
              )}
            </div>
            ...
            <div className="divide-y divide-slate-100 flex-1">
              {calendarFilteredEntries.length > 0 ? (
                ...
                <div className="p-8 text-center text-xs text-slate-400 font-medium">
                  {selectedDate ? "No races scheduled on this date." : "No registered races found."}
                </div>
              )}
            </div>
          </div>
