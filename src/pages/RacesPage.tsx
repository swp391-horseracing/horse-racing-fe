  ...
  return (
    <button
      ...
      className={`group w-full flex items-center justify-between px-5 py-4 text-left transition-all border-l-4 ${
        selected ? "bg-primary/5 border-l-primary" : isLive ? "bg-secondary/5 border-l-secondary hover:bg-secondary/10" : "border-l-transparent hover:bg-slate-50/50"
      }`}
    >
      <div className="flex items-center gap-4 min-w-0">
        <span className={`font-mono text-base tracking-tight font-black ${selected ? "text-primary" : "text-muted-foreground"}`}>{race.time}</span>
        <div className="truncate">
          <p className={`font-bold font-headline text-base truncate ${selected ? "text-primary" : "text-foreground"}`}>{race.title}</p>
          <p className="text-xs text-muted-foreground mt-1">{race.className} · {race.distance} · {race.surface}</p>
        </div>
      </div>
...
export default function RacesPage() {
  ...
  const raceDays = useMemo(() => baseRaces.map((r) => parseLocalDate(r.date)), [baseRaces]);
  ...
  return (
    <div className="h-full w-full overflow-y-auto bg-background custom-scrollbar">
      <div className="mx-auto max-w-[1400px] w-full px-4 md:px-6 py-6 md:py-8">
        <div className="flex-shrink-0">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <div className="flex items-center gap-3.5 min-w-0">
              ...
              <div className="min-w-0">
                {tournamentName && <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-1 truncate">{tournamentName}</p>}
                <h1 className="text-3xl font-black font-headline text-primary tracking-tight leading-none truncate">{tournamentId ? "Race Roster" : "All Races"}</h1>
              </div>
            </div>
          ...
          <div className="mb-6 grid grid-cols-2 md:grid-cols-4 gap-3 lg:gap-4">
            {(["All", "Live", "Upcoming", "Completed"] as const).map((key) => {
              const labels = { All: "Total", Live: "Live", Upcoming: "Upcoming", Completed: "Completed" };
              return (
                <ScheduleStatCard
                  key={key}
                  label={labels[key]}
        ...
        <ScheduleLayout
          panelOpen={panelOpen}
          leftColClass={isCalendarMode ? (panelOpen ? "lg:col-span-4" : "lg:col-span-12") : (panelOpen ? "lg:col-span-3" : "lg:col-span-12")}
                  ...
                  <div className="divide-y divide-border flex-1">
                    {calendarFilteredRaces.length > 0 ? (
                      calendarFilteredRaces.map((race) => (
                        <RaceRow key={race.id} race={race} selected={selectedRace?.id === race.id} onClick={() => handleSelectRace(race)} />
                      ))
                    ) : (
                      <div className="p-12 text-center text-sm text-muted-foreground font-medium">No official races slated for this calendar day.</div>
                    )}
                  </div>
                </div>
              )
              ...
              >
                <div>
                  ...
                  {horseList && horseList.length > 0 ? (
                    ...
                    <div className="rounded-2xl border border-dashed border-border py-10 text-center text-sm text-muted-foreground font-medium bg-background">No verified roster loaded. Final draws unlisted.</div>
                  )}
                </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                    {officials.map((o) => (
                      <div key={o.initials} className="flex items-center gap-4 rounded-xl border border-border p-4 bg-background">
                        <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg shadow-sm bg-card text-primary text-sm font-black border border-border">{o.initials}</div>
                        <div className="min-w-0">
