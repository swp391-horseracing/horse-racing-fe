  ...
  return (
    <section className="flex h-full w-full flex-col gap-6 p-6">
      <header className="space-y-1">
        <h1 className="text-3xl font-extrabold text-[#064E3B]">Global Leaderboard</h1>
        <p className="text-sm text-slate-600 max-w-2xl">
          The definitive ranking of the world's most elite equestrian talent. Performance data is updated in real-time.
        </p>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[top3[1], top3[0], top3[2]].filter(Boolean).map((row, i) => {
          ...
          return (
            ...
            >
              <div className="flex flex-col items-center gap-3">
                {isCenter ? <Crown className="h-6 w-6 text-amber-300" /> : <div className="h-6 w-6" />}
                <div
                ...
                >
                  {horse.image ? (
                    ...
                    <div className="h-full w-full bg-slate-100 flex items-center justify-center text-xs text-slate-400">No Image</div>
                  )}
                </div>

                <div className="text-center">
                  <div className={`text-xl font-bold ${isCenter ? "text-white" : "text-slate-900"}`}>{horse.name}</div>
                  <div className={`text-xs mt-1 ${isCenter ? "text-amber-200" : "text-slate-500"}`}>{horse.owner}</div>
                </div>
                ...
                <div className="mt-2 w-full flex flex-col sm:flex-row sm:justify-between items-center text-xs gap-2">
                  <div className="text-center">
                    ...
                    <div className={`font-bold ${isCenter ? "text-emerald-200" : "text-slate-700"}`}>{formatWinRate(horse.performance)}</div>
                  </div>
                  <div className="text-center">
                    <div className="text-[10px] text-slate-400">Earnings</div>
                    <div className={`font-bold ${isCenter ? "text-amber-200" : "text-slate-700"}`}>{formatEarnings(horse.performance)}</div>
                  </div>
                  <div className="text-center">
                    <div className="text-[10px] text-slate-400">Rank</div>
                    <div className={`font-bold ${isCenter ? "text-white" : "text-slate-700"}`}>{row.rank}</div>
                  </div>
                </div>
              </div>
            </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div className="overflow-x-auto">
          <div className="flex items-center justify-between mb-3">
            <div className="text-sm text-slate-600">Showing {(page - 1) * pageSize + 1}–{Math.min(page * pageSize, sortedRows.length)} of {sortedRows.length}</div>
            ...
            <tbody>
              {paginatedRows.map((r) => (
                <tr key={r.horse.id} className="border-t border-slate-100">
                  ...
                  <td className="py-3">{formatEarnings(r.horse.performance)}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="mt-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setPage((p) => Math.max(1, p - 1))} className="inline-flex items-center gap-1 rounded px-3 py-1 border bg-white">
              <ChevronLeft className="h-4 w-4" />
              Prev
            </button>
            <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} className="inline-flex items-center gap-1 rounded px-3 py-1 border bg-white">
              Next
