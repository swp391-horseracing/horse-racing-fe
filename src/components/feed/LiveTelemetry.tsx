...
export function LiveTelemetry() {
  ...
  const [, setElapsedSeconds] = useState(38);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setCurrentPace((prev) =>
        Math.min(72, Math.max(50, Math.round((prev + paceChange) * 10) / 10))
      );
      ...
      setLeaders((prev) => {
        const next = prev.map((leader) => {
          ...
          const speed =
            Math.round(
              Math.min(75, Math.max(55, leader.speed + deltaSpeed)) * 10
            ) / 10;
          return { ...leader, speed };
        });
        ...
        return next.map((leader, index) => {
          const gap =
            index === 0
              ? "-"
              : `+${(index * 0.4 + Math.random() * 0.3).toFixed(1)}L`;
    ...
    <div className="flex-1 overflow-y-auto p-6 max-w-7xl w-full mx-auto font-body h-full min-h-0">
      <div className="bg-white border border-[#064E3B]/10 rounded-2xl p-6 flex flex-col shadow-sm">
        <div className="flex justify-between items-end mb-6 border-b border-slate-100 pb-4">
          <div>
            <h3 className="font-headline font-bold text-2xl text-[#064E3B] m-0">
              Live Telemetry
            </h3>
          ...
          <div className="flex gap-4">
            <div className="text-right">
              <span className="block font-label text-[9px] font-bold text-slate-400 uppercase tracking-wider">
                Distance Remaining
              </span>
              <span className="font-headline text-lg font-black text-[#064E3B]">
                {distanceRemaining}m
              </span>
            </div>
        ...
        <div className="space-y-2 flex-1">
          ...
          {leaders.map((leader) => (
            ...
            >
              <div
                className={`col-span-1 text-center font-headline font-black text-sm ${
                  leader.pos === 1 ? "text-[#EAB308]" : "text-slate-400"
                }`}
              >
                {leader.pos}
              </div>
              <div className="col-span-1 font-label text-xs font-bold text-slate-500">
                {leader.no}
              </div>
              <div className="col-span-4 flex items-center gap-3">
                ...
                <div>
                  <div className="font-bold text-slate-700 text-xs leading-tight">
                    {leader.name}
                  </div>
              ...
              <div className="col-span-2 text-right font-label text-xs font-bold text-slate-700">
                {leader.speed} km/h
              </div>
              <div className="col-span-2 text-right font-label text-xs font-bold text-slate-400">
                {leader.gap}
              </div>
              <div className="col-span-2 text-right font-label text-xs font-extrabold text-[#064E3B]">
                {leader.odds}
              </div>
            </div>
          ))}
