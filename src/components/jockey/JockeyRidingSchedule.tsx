...
import { Search, CalendarDays, Flag, Users, Clock, Compass, Trophy, Activity } from "lucide-react";
import { ScheduleLayout } from "../schedule/ScheduleLayout";
import { ScheduleCalendar } from "../schedule/ScheduleCalendar";
import { ScheduleStatCard } from "../schedule/ScheduleStatCard";
import { ScheduleDetailFrame, type TabConfig } from "../schedule/ScheduleDetailFrame";
import { cn } from "../../lib/utils";
import {
  type MyRide,
  type ComputedRideStatus,
  type RideDetailTab,
  getComputedRideStatus,
  formatOrdinal,
  officialsMock,
} from "../../pages/JockeyPage";
...
export default function JockeyRidingSchedule({ rides, onAcceptRide, onDeclineRide }: RidingScheduleProps) {
  ...
  const raceDays = useMemo(() => rides.map((r) => {
      const d = new Date(r.scheduledAt);
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 max-w-7xl w-full mx-auto font-body h-full custom-scrollbar bg-[#F4F6F5]">
      <div className="flex-shrink-0">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-xs text-slate-500 font-semibold mt-1">Your confirmed race assignments and schedule overview</p>
          </div>
          <div className="relative w-full sm:w-72 shadow-sm rounded-xl border border-slate-200 bg-white">
            <Search className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
            <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search races, horses..." className="w-full h-10 rounded-xl bg-transparent pl-10 pr-4 text-xs font-medium outline-none transition focus:border-[#064E3B] placeholder:text-slate-400" />
          </div>
        </div>
        ...
        <div className="mb-6 grid grid-cols-2 md:grid-cols-5 gap-3">
          {(["All", "pending", "accepted", "declined", "finished"] as const).map((key) => {
            ...
            return (
              <ScheduleStatCard key={key} label={labels[key]} value={counts[key]} active={statusFilter === key} onClick={() => setStatusFilter(key)} liveDot={key === "pending"} />
            );
          })}
        </div>
      </div>
      ...
      <ScheduleLayout
        panelOpen={activeSelectedRide !== null}
        calendarSlot={<ScheduleCalendar selectedDate={selectedDate} onSelect={setSelectedDate} raceDays={raceDays} />}
        listSlot={
          <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden flex flex-col">
            <div className="border-b border-slate-100 bg-[#F4F6F5] px-5 py-3 flex items-center gap-2">
              ...
              <span className="text-[10px] font-black uppercase tracking-wider text-slate-500">
                {selectedDate ? `Schedule for ${selectedDate.toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}` : "All Scheduled Assignments"}
              </span>
            </div>
            <div className="divide-y divide-slate-100 flex-1">
              {calendarFilteredRides.length > 0 ? (
                calendarFilteredRides.map((ride) => (
                  <RideRow key={ride.id} ride={ride} selected={activeSelectedRide?.id === ride.id} onClick={() => handleSelectRide(ride)} />
                ))
              ) : (
                <div className="p-8 text-center text-xs text-slate-400 font-medium">{selectedDate ? "No assigned races for this day." : "No assigned races found."}</div>
              )}
            </div>
          </div>
        }
        detailSlot={
          activeSelectedRide && <RidingScheduleDetailPanel ride={activeSelectedRide} onClose={() => setSelectedRide(null)} onAccept={onAcceptRide} onDecline={onDeclineRide} />
        }
      />
    </div>
  );
...
// Sub-components
function RidingScheduleDetailPanel({ ride, onClose, onAccept, onDecline }: { ride: MyRide; onClose: () => void; onAccept: (id: string) => void; onDecline: (id: string) => void; }) {
            ...
            <div className="bg-white border border-[#064E3B]/10 rounded-xl overflow-hidden shadow-sm">
              <div className="divide-y divide-slate-100 text-sm">
                <div className="flex items-center justify-between px-5 py-3">
                  <span className="text-slate-500 flex items-center gap-2 font-medium"><Compass className="w-4 h-4 text-slate-400" /> Distance</span>
                  <span className="font-bold text-slate-800">{ride.distanceMeters}m</span>
                </div>
                <div className="flex items-center justify-between px-5 py-3">
                  <span className="text-slate-555 flex items-center gap-2 font-medium"><Activity className="w-4 h-4 text-slate-400" /> Going</span>
                  <span className="font-bold text-slate-800 capitalize">{ride.trackCondition}</span>
                </div>
                <div className="flex items-center justify-between px-5 py-3">
                  <span className="text-slate-555 flex items-center gap-2 font-medium"><Users className="w-4 h-4 text-slate-400" /> Field Size</span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {officialsMock.map((o) => (
                <div key={o.initials} className="flex items-center gap-3.5 rounded-xl border border-[#064E3B]/10 p-3.5 bg-[#F4F6F5]/30">
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg shadow-sm bg-white text-[#064E3B] text-xs font-black border border-[#064E3B]/10">{o.initials}</div>
                  ...
                  return (
                    <tr key={idx} className={cn("transition-colors", isOurs ? "bg-[#064E3B]/5 font-semibold" : "hover:bg-slate-50/50")}>
                      <td className="px-5 py-3.5 text-center">
                        <span className={cn("flex h-6 w-6 items-center justify-center rounded-md border text-xs font-black mx-auto shadow-sm", isOurs ? "bg-[#064E3B] text-white border-[#064E3B]" : "bg-white text-slate-700 border-slate-200")}>{runner.cloth}</span>
                      </td>
...
function RideStatusBadge({ status, onDark }: { status: ComputedRideStatus; onDark?: boolean; }) {
  if (onDark) {
    const styles = { pending: "bg-[#D97706] !text-white border-transparent", accepted: "bg-emerald-600 !text-white border-transparent", declined: "bg-rose-600 !text-white border-transparent", finished: "bg-slate-600 !text-white border-transparent" };
    return <span className={cn("px-2.5 py-0.5 rounded-[4px] text-[9px] font-black uppercase tracking-wider border shadow-sm !text-white", styles[status])}>{status}</span>;
  }
...
function RideRow({ ride, selected, onClick }: { ride: MyRide; selected: boolean; onClick: () => void; }) {
  const computedStatus = getComputedRideStatus(ride);
  return (
    <button onClick={onClick} className={`group w-full flex flex-col px-4 py-3.5 text-left transition-all border-l-4 ${selected ? "bg-[#064E3B]/5 border-l-[#064E3B]" : computedStatus === "pending" ? "bg-[#EAB308]/5 border-l-[#EAB308] hover:bg-[#EAB308]/10" : "border-l-transparent hover:bg-slate-50"}`}>
      <div className="flex items-center justify-between w-full">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="truncate">
            ...
            <p className="text-[10px] text-slate-505 mt-0.5 truncate font-medium">Ride: <span className="text-slate-700 font-bold">{ride.ride}</span> • Gate {ride.laneNumber}</p>
          </div>
        </div>
