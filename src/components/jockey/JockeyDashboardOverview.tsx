...
import { Icons, type JockeyHorse } from "../../pages/JockeyPage.tsx";
...
export default function JockeyDashboardOverview({ data, setActiveTab, horseList }: DashboardOverviewProps) {
                ...
                <div key={horse.id} className="flex items-center justify-between p-2.5 rounded-xl border border-slate-100 bg-slate-50/50 hover:border-slate-200 transition">
                  <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    <div className="flex flex-col min-w-0">
                      ...
                      <span className="text-[10px] text-slate-455 truncate">{horse.breed || "Thoroughbred"} • {horse.gender || "Gelding"}</span>
                    </div>
                  </div>
                  <div className="text-right shrink-0">
                    <span className="text-xs font-bold text-[#064E3B] font-label block">{horse.owner || horse.ownerId}</span>
                    <span className="text-[9px] text-slate-400 font-semibold block">{horse.status || "Active"}</span>
                  </div>
                </div>
              ))}
            </div>
