...
import { type FilterType, Icons, statusConfig } from "../../pages/JockeyPage";
...
export default function InvitationsView({ data, selectedId, setSelectedId, onAccept, onDecline }: InvitationsViewProps) {
      ...
      <div className="w-96 shrink-0 border-r border-[#064E3B]/10 bg-white flex flex-col h-full overflow-hidden">
        <div className="p-4 border-b border-slate-100 space-y-3.5 shadow-sm">
          ...
          <div className="flex gap-1.5 overflow-x-auto pb-1 scrollbar-none">
            {filters.map((f) => (
              <button key={f} onClick={() => setFilter(f)} className={cn("rounded-xl px-3 py-2 text-xs font-bold whitespace-nowrap", filter === f ? "bg-[#064E3B] text-white" : "bg-[#F4F6F5] text-slate-500")}>{f}</button>
            ))}
          </div>
        </div>
...
function InvitationDetail({ inv, onAccept, onDecline }: { inv: Invitation | null; onAccept: (id: number) => void; onDecline: (id: number) => void; }) {
  if (!inv) {
    return (
    ...
    <div className="p-6 h-full overflow-y-auto space-y-6 font-body">
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 border-b border-[#064E3B]/10 pb-5">
        <div>
          ...
          <p className="text-xs font-semibold text-slate-500">{inv.tournament}</p>
        </div>
      ...
      <div className="bg-white border border-[#064E3B]/10 rounded-2xl p-5 space-y-4 shadow-sm">
        <div className="grid grid-cols-2 gap-4">
          <div><p className="text-[9px] text-slate-400 font-bold uppercase">Owner</p><p className="text-sm font-semibold text-slate-800">{inv.owner}</p></div>
          <div><p className="text-[9px] text-slate-400 font-bold uppercase">Race Time</p><p className="text-sm font-semibold text-slate-800">{inv.raceTime}</p></div>
        </div>
      </div>
      ...
      {isPending ? (
        ...
        <div className="bg-white border border-[#064E3B]/10 rounded-2xl p-6 text-center shadow-sm">
          <div className="flex h-12 w-12 items-center justify-center rounded-full bg-slate-50 border border-slate-100 text-slate-455 mx-auto"><Icons.Lock /></div>
